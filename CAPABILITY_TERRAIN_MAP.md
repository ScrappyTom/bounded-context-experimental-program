...
This map reorganizes the existing evidence by information-processing function
rather than by mechanism name. It is deliberately small. Its purpose is to
show what has actually been observed, what recurs across independent worlds,
and where the program remains too narrow to justify optimization.

The source of truth for claim scope remains `EVIDENCE_LEDGER.md`. Heterogeneous
tasks, models, interfaces, horizons, and evaluators are not pooled. A mark of
recurrence means a broadly similar functional boundary appeared in independent
worlds; it is not a causal estimate or architecture promotion.

Evidence imported from `bounded-context-harness-effects` uses Qwen 3.6 27B,
mostly rule-dense JavaScript coding tasks, a larger context regime, and
different action interfaces. It supplies cross-profile functional analogues
and within-program task recurrence. It is not pooled replication of the
Qwen3.8 large-world studies and does not transfer treatment effects,
prevalence estimates, or model-specific capability claims.

## Independence groups represented so far

| Group | Representative evidence | Relationship to other groups |
|---|---|---|
| Navigation synthesis world | E03–E26, E33–E34 | Many seeds and packet variants share one task/world and donor lineage. They count as deep local evidence, not independent-world breadth. |
| Older code false-closure and repair worlds | E27–E30 | Multiple pinned tasks and studies imported through the prior-evidence audit. Apparatus differs from the standalone sequence. |
| Older research and phase-reentry worlds | E30 | Multiple code/research workflows with task-authored boundaries. Independent of the navigation world but heterogeneous internally. |
| Large-document exact-handoff worlds | E31 | Two related maintenance trajectories; close transfer rather than broad independence. |
...
| Function | Literal evidence | Independent recurrence | Current disposition |
|---|---|---|---|
...
| Prospective capacity safety | Small first-fit demotions admitted blocked results in E03; repeated relief maintained the navigation horizon in E04; E36 repeated the operation on a fresh Ceiba world. E40 exposed one tiny exact body whose receipt cost 79 more prompt tokens. | Yes: navigation and Ceiba are independent task worlds. | Qualified mechanical baseline with a corrected eligibility law: prospective rendered savings must be strictly positive. Further ordering or same-boundary horizon tuning is low priority. |
...
| Relationship formation and qualification capture | The navigation candidate became a strong partial; one fresh research control produced a substantial 9/12 memo; E37 D2 integrated three exact records but lost one qualification. In E40, bounded work records expressed cross-source relationships and cited 13 sources over time, yet each final record retained only four citations. E46 produced two strong-partial exact packages; coupling broadened final citation but retained a stale roadmap. Sister semantic ledgers and relation surfaces could express compact relationships but sometimes changed exact values or overloaded later expression. | Suggestive across profiles and tasks; E40 adds active formation/attrition and E46 adds durable artifact binding with anchoring in an independent world. | Separate formation, retention, provenance, artifact binding, correction, and later use. Exact persistence can preserve useful relations and stale commitments. |
...
| Model-authored semantic persistence | Qwen3.8 source digest increased short-horizon recovery; a progress package caused premature closure; E40 sidecars lost prior citation breadth. E46 coupled semantic integration to exact work and accelerated construction. E52 produced stronger coupled work but only 5/18 maintenance outputs qualified per arm; A1 preserved source-reversing errors. E57 reconstructed 57 trajectory maintenance calls: complete replacements produced eight accepted byte-identical no-ops across E46/Cedar, 29 carrier-budget failures across E46/Cedar, stale-state capitalization in E46, and unsupported completion in Bluehaven. E61 produced materially safe source-local meaning but its lexical safety rule rejected exact owner-source relationships to four other objects. E63 mechanically separates source-slot authority, exact evidence, relation referents, derived support, and currentness across ten historical fixtures; two provenance-valid semantic reversals remain failures. E64 embeds that distinction in a fresh complete W0/L1 apparatus while adding no live behavior. The sister coverage ledger was used in 6/6 cells while artifacts fell from 6/6 to 3/6. | Behavioral activity and risk recur across tasks and profiles. Literal unsupported completion remains one-world evidence, complete-global replacement brittleness recurs through several failure surfaces, and E61 adds a safety-policy × relationship-topology conflict. E63–E64 establish apparatus coherence and fresh-system reachability, not recurrence or utility. | Close repeated complete-global replacement and lexical identity isolation as default semantic-scope rules. Aster may test provenance-local source reports plus separately supported derived work, with independent semantic review and at most one carrier-expression gate after authentic common pressure. |
...
Six conclusions are now comparatively broad:

1. Exact custody and prospective capacity safety are reusable mechanical
   substrate functions.
2. Operability does not establish relationship assembly, action formation,
...
- how dispersed evidence becomes a durable relationship or claim and then
  actually constrains action;
- whether incremental exact work that survives information turnover improves
  candidate-bound quality and closure rather than only construction timing;
- which conditions let the actor stop acquisition, organize work, and express
  a legal action;
- how effects and checks remain current and discriminating across continuation;
...
- frontier schema repair;
- one-source digest format tweak;
...
apparatus gate and is closed under E38's stopping rule. E39/E40 then measured
bounded ingress × semantic-work sidecar under live pressure. Both functions
activated, but exact task-artifact accumulation did not: semantic grounding
contracted, batch geometry accelerated reentry use, and no candidate changed.
...
its call budget after accepting only 5/18 updates per arm. E53–E56 added a
fresh Bluehaven transport case in which partial-evidence complete replacement
invented unseen-source substance.

E57 reconciled those worlds rather than promoting the Bluehaven failure into a
generic maturity controller. The literal invention did not recur, but the
complete-global replacement operation failed recurrently through accepted
no-ops, carrier loss, stale-state persistence, and error capitalization. The
selected checkpoint is now the fresh source-local evidence-delta × direct
exact-work whole-system Stage 0 in
`NEXT_SYSTEM_INTERACTION_EVIDENCE_DELTA_SCOUT.md`. It retains pressure, exact
work, effects, current checks, repair, recheck, and closure in both arms;
...
The sister-program audit was performed against
`bounded-context-harness-effects` commit
`5fc4f050f3de176a023019df24cec4a5d5781e71`. Principal sources were:

- `evidence/model-psychology/qwen36-27b.md`;
- `evidence/101_102_native_ecology_cross_study_synthesis.md` at historical
  result commit `ac2fd28e`;
- `evidence/105_111_transition_boundary_audit_synthesis.md` at `9f59dc2f`;
- `evidence/105_112_action_organization_audit_synthesis.md` at `3e536fb0`;
- `evidence/109_model_authored_coverage_ledger_synthesis.md` at `d9906e14`;
- `evidence/110_mechanical_check_provenance_frozen_states_synthesis.md` at
  `d7d68f5c`;
- `evidence/111_evidence_bound_relation_action_binding_synthesis.md` at
  `7bf3e544`;
- `evidence/112_mechanical_fresh_response_allocation_synthesis.md` at
  `da519f4b`; and
- `evidence/113_mechanical_incomplete_payload_suffix_synthesis.md` at
  `040bfdcf`.

The 102- and 114-episode audits are selected or enriched corpora, not 102 or
114 independent worlds. Experiments 104 and 106 reused earlier tasks;
Experiments 110–112 reused earlier frozen states. These sources sharpen probe
questions; they do not manufacture transfer.

## Completed offline refinement

`E36_DECISION_GEOMETRY_AUDIT.md` now supplies the representative
decision-level reconstruction for the pressure-qualified Ceiba trajectory. It
records:

- independence group and work mode;
- accessible, observed, and literal-at-decision information;
- semantic or work-product residue;
- action required and action taken;
- effect and effect uptake;
- broad endpoint and competing explanations;
